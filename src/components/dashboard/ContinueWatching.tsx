    ...
    <section className="mt-10">
      ...
      {loading ? (
        ...
        <div
          className={
            rest.length > 0
              ? 'grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,300px)] lg:gap-12'
              : 'grid'
          }
        >
          <FeatureCard group={lead} onPlay={handlePlay} />
          {rest.length > 0 && (
            <div className="lg:border-s lg:border-border lg:ps-12">
      ...
      <div className="grid sm:grid-cols-[minmax(0,42%)_minmax(0,1fr)]">
        <div className="relative aspect-video w-full overflow-hidden bg-background">
          <LocalThumbnail
            ...
            iconClassName="h-7 w-7 text-text-faint"
            fallbackClassName={QUIET_FALLBACK}
          />
          {/* Settles the still into the card instead of letting it sit on top
              of it. Token-derived, so it darkens on onyx and lightens on pearl. */}
          <span
            aria-hidden="true"
            className="pointer-events-none absolute inset-0"
            style={{
              background:
                'radial-gradient(120% 90% at 50% 40%, transparent, rgb(var(--bg-main-rgb) / 0.55))',
            }}
          />
          {canPlay && (
            <span className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <span
                className="flex h-14 w-14 items-center justify-center rounded-full border border-accent-gold/40 text-accent-gold motion-safe:transition-transform motion-safe:duration-300 motion-safe:ease-out motion-safe:group-hover:scale-110"
                style={{ background: 'rgb(var(--bg-main-rgb) / 0.62)' }}
              >
          ...
          <span
            ...
            style={{ background: 'rgb(var(--bg-main-rgb) / 0.55)' }}
          >
            <span
        ...
        <div className="flex min-w-0 flex-col justify-center gap-3 p-6 sm:p-8">
          ...
          {canPlay && (
            <span className={`${eyebrow} mt-3 flex items-center gap-2 text-accent-gold`}>
              {t('continue')}
              <span className="tabular-nums text-text-faint" dir="ltr">
...
const FeatureSkeleton: React.FC = () => (
  <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,300px)] lg:gap-12">
